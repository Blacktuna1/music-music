    ...
    <div class="musicList">
      <el-tabs value="first" @tab-click="clickTab">
        <el-tab-pane label="歌曲列表" name="first">
          <!-- 表格 -->
          <!-- 绑定data -->
          <!-- 监听两种事件：双击and单击，row-dblclick and cell-click-->
          <!-- 双击事件，返回提交一个row-key -->
          <!-- stripe设置斑马纹样式 -->
          <el-table
...
export default {
  ...
  methods: {
    ...
    // 获取歌单评论
    // 类似的，根据id和时间戳等信息查询歌单的评论
    async getMusicListComment(type) {
      // 获取时间戳
          //类似的，根据id、时时间戳等信息，请求歌单的评论
      var timestamp = Date.parse(new Date());
    ...
    // 获取歌曲详情
      // 通过id请求歌曲信息，这里应该是在双击事件中调用，将元素添加到MusiclistDetail
      // 这里的详细信息貌似不包括歌曲的url
      // 看来最重要的是将歌曲id传给Bottom，bottom是一个完整的播放器，播放器的功能由bottom实现
